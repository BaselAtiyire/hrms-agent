# ─────────────────────────────────────────────────────────────────────
# HRMS — AWS Production Infrastructure
# Stack: ECS Fargate (single replica) + EFS (SQLite) + ALB + ECR
# Region: us-east-1
#
# Usage:
#   terraform init
...
variable "desired_count"   { default = 1 }   # SQLite: keep at 1
...
# NAT Gateway for private subnet egress (pulling ECR images, etc.)
...
resource "aws_lb_listener" "http" {
  ...
  # Redirect to HTTPS if you have a cert; forward directly otherwise
  default_action {
    type             = "forward"
...
resource "aws_ecs_service" "hrms" {
  ...
  depends_on = [
    ...
    aws_efs_mount_target.hrms_data,
  ]
...
output "alb_dns_name" {
  description = "ALB public DNS — point your domain here"
  value       = aws_lb.hrms.dns_name
}

output "ecr_repository_url" {
  description = "ECR repo URL for docker push"
  value       = aws_ecr_repository.hrms.repository_url
}

output "ecs_cluster_name" {
  value = aws_ecs_cluster.hrms.name
}

output "ecs_service_name" {
  value = aws_ecs_service.hrms.name
}

output "cloudwatch_log_group" {
  value = aws_cloudwatch_log_group.hrms.name
}
